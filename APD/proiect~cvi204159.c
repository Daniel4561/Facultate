	...
	switch (event)
	{
		...
		case EVENT_COMMIT:
			...
			SetCtrlAttribute(panel, PANEL_2_GRAPH_3, ATTR_XAXIS_OFFSET, (double)start2 * sampleRate);
				...
				GetCtrlVal(panel,  PANEL_2_BINARYSWITCH_TYPE, &val);
				WindowConst windowConst;
				N=44100;
				
				
					
				if(npoints - N*stop2 > N){
					
					Copy1D(waveData+start2*N, N, ferestruire);
				}
				else
				{
					for(int i=N*stop2;i<npoints;++i)
					{
						ferestruire[i-N*stop2] = waveData[i];
					}
					for(int i = npoints-N*stop2;i<N;++i)
					{
						ferestruire[i]=0;
					}
				...
				Copy1D(ferestruire, N, vector);
				
				SavitzkyGolayFiltering(vector, N, 3, 100, NULL, savitsky);
				
				
				//tipul de fereastra
				if(val==0)
				{
					ScaledWindow(ferestruire, N, RECTANGLE_, &windowConst);
					ScaledWindow(savitsky, N, RECTANGLE_, &windowConst);
									
				}
				if(val==1)
				{
					ScaledWindow(ferestruire, N, BLKMAN_, &windowConst);
					ScaledWindow(savitsky, N, BLKMAN_, &windowConst);
				}
				...
				AutoPowerSpectrum(ferestruire, N, 1.0/sampleRate, spectru_semnal, &df);
				double freqPeak, powerPeak;
				PowerFrequencyEstimate (spectru_semnal, N/2, -1.0, windowConst, df, 7, &freqPeak,&powerPeak);
				SpectrumUnitConversion(spectru_semnal, N/2, 0, SCALING_MODE_LINEAR, DISPLAY_UNIT_VPK, df, windowConst, spectru_semnal, unit);
				...
				norch1[0]=vector[0]; norch1[1]=vector[1];// wfm – semnalul dat, wfm3 rezultatul filtrarii
				for(int i=2;i<N;i++) //dimensiunea bufferului =1024 sau se inlocuieste
				//cu numarul de esantioane
				norch1[i]=Numerator[0]*vector[i]+Numerator[1]*vector[i-1]+Numerator[2]*vector[i-2] -
				Denominator[1]*norch1[i-1]-Denominator[2]*norch1[i-2];
				
				Copy1D(norch1, N, norch);
				
				//tipul de fereastra
				if(val==0)	
				{
					ScaledWindow(norch, N, RECTANGLE_, &windowConst);
									
				}
				if(val==1)
				{
					ScaledWindow(norch, N, BLKMAN_, &windowConst);
				}
				//spectre
				spectru_savitsky = (double*)calloc(N/2, sizeof(double));
				AutoPowerSpectrum(savitsky, N, 1.0/sampleRate, spectru_savitsky, &df);
				PowerFrequencyEstimate (spectru_savitsky, N/2, -1.0, windowConst, df, 7, &freqPeak,&powerPeak);
				SpectrumUnitConversion(spectru_savitsky, N/2, 0, SCALING_MODE_LINEAR, DISPLAY_UNIT_VPK, df, windowConst, spectru_savitsky, unit);
				
				spectru_notch = (double*)calloc(N/2, sizeof(double));
				AutoPowerSpectrum(norch, N, 1.0/sampleRate, spectru_notch, &df);
				PowerFrequencyEstimate (spectru_notch, N/2, -1.0, windowConst, df, 7, &freqPeak,&powerPeak);
				SpectrumUnitConversion(spectru_notch, N/2, 0, SCALING_MODE_LINEAR, DISPLAY_UNIT_VPK, df, windowConst, spectru_notch, unit);
				...
				PlotY(panel, PANEL_2_GRAPH_6, spectru_semnal, N/2, VAL_DOUBLE, VAL_THIN_LINE, VAL_EMPTY_SQUARE, VAL_SOLID, VAL_CONNECTED_POINTS, VAL_GREEN);
				
				
				DeleteGraphPlot(panel, PANEL_2_GRAPH_4, -1, VAL_IMMEDIATE_DRAW);
				PlotY(panel, PANEL_2_GRAPH_4, spectru_savitsky, N/2, VAL_DOUBLE, VAL_THIN_LINE, VAL_EMPTY_SQUARE, VAL_SOLID, VAL_CONNECTED_POINTS, VAL_GREEN);
				
				DeleteGraphPlot(panel, PANEL_2_GRAPH_5, -1, VAL_IMMEDIATE_DRAW);
				PlotY(panel, PANEL_2_GRAPH_5, spectru_notch, N/2, VAL_DOUBLE, VAL_THIN_LINE, VAL_EMPTY_SQUARE, VAL_SOLID, VAL_CONNECTED_POINTS, VAL_GREEN);*/
			break;
	}
	return 0;
}
							 ...
							 void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
	{
		case EVENT_COMMIT:
			...
			{
				for(int i = sampleRate-N*start3 ;i<sampleRate;++i)
				{
					sec_semnal[i-sampleRate-N*start3] = ferestruire[i];
				}
				for(int i = sampleRate-N*start3;i<N;++i)
				{
					sec_semnal[i] = 0;
				}
			}
						 ...
						 void *callbackData, int eventData1, int eventData2)
{
	...
	{
		case EVENT_COMMIT:
			if(loaded3 != 0 && sampleRate/N>=start3)
			{
				start3++;
				stop3++;
				SetCtrlVal(panel, PANEL_2_NUMERIC_3, stop3);
